...
class ExportCSPreferences(AddonPreferences):
  ...
  def draw(self, context):
    ...
    row = layout.row(align=True)
    box = row.box()
    box.template_list("UI_UL_list", "files", self, "shaderSetFiles", self, "active_shaderSetFiles_index", rows=5, maxrows=5, type='DEFAULT')
    sub = row.column(align=True)
    sub.operator("addonpreferences.shadersetfile_add", icon='ZOOMIN', text="")
    sub.operator("addonpreferences.shadersetfile_remove", icon='ZOOMOUT', text="")
