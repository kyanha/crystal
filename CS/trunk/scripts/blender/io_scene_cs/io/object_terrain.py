...
def exportHeightMap(object):
  ...
  # Make render scene
  render_scene = bpy.data.scenes.new(object.uname+'_'+'terrain_render_scene') 
  ...
  render_scene['newScene'] = False #Make sure the handelers don't reset the engine.
  ...
  # Create Material
  # The texture
  hm_tex=bpy.data.textures.new(name=object.uname+'_'+"hm_tex", type="BLEND")
  ...
  # Assign
  #bpy.ops.object.material_slot_add() #This will fail, so lets assume the user has already added a material slot
  ...
  hm_camera.ortho_scale=max(odim[0:1]) # max of the two dimensions of the outdoor mesh
  hm_camera.clip_start=0.001
  hm_camera.clip_end=1.5*(oloc[2]+odim[2])
  # Object
  hm_camera_ob=bpy.data.objects.new(object.uname+'_'+"hm_camera", hm_camera)
