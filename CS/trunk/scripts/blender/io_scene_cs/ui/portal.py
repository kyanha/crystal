...
class SelectDestScene(bpy.types.Operator):
    ...
    def avail_scenes(self,context):
        items = [(str(i),s.name,s.name) for i,s in enumerate(bpy.data.scenes)]
        items.append((str(-1),' NONE','None'))
        return sorted(items, key=operator.itemgetter(1))
    select_scenes = bpy.props.EnumProperty(items = avail_scenes, name = "Available scenes")
...
class OBJECT_OT_csCreatePortal(bpy.types.Operator):
  ...
  def invoke(self, context, event):
    ...
    # Create a new object
    new_obj = bpy.data.objects.new(name, new_mesh)   # link the mesh data to the object
