...
def IDRefContainer(cls):
    idref_items = [(attr, item) for attr, item in cls.__dict__.items() if isinstance(item, IDRefProperty)]
...
def draw_idref(layout, data, prop):
    ...
    row.prop_search(data, "%s__name__" % prop, bpy.data, get_idtype_list_prop(idtype), text="")
