...
def CameraAsCS (self, func, depth, camera=None):
  ...
  if camera == None:
    ...
    func(' '*depth +'  <forward y="0" x="0" z="1" />')
    func(' '*depth +'</start>')
  else:
    ...
    func(' '*depth +'  <up y="%f" x="%f" z="%f" />'%tuple(rot * mathutils.Vector((0.0, 0.0, 1.0))))
    func(' '*depth +'  <forward y="%f" x="%f" z="%f" />'%tuple(rot * mathutils.Vector((-1.0, 0.0, 0.0))))
    func(' '*depth +'</start>')
